...
Module Module1
    ...
    Public Function strconnection() As String
        ...
        str = "server=localhost; uid=root; password=''; database=accountingsystem;convert Zero Datetime=true"
    ...
    '    Try
    '        sqlcon.ConnectionString = "server=localhost;uid=root;password='';database=accountingsystem;convert Zero DateTime=true"
    ...
    Public Sub ModifyComboBox(query As String, combobox As Bunifu.Framework.UI.BunifuDropdown)

        FillDataTable(query)
        combobox.Clear()

        For i As Integer = 0 To dt.Rows.Count - 1
            combobox.AddItem(dt.Rows(i).Item(0))
        Next
        sqlcon.Close()

    End Sub

    Public Function FillDataTable(query As String)
        dt = New DataTable
        dt.Clear()
    ...
    Public Sub FillDataTable(query As String, datatable As DataTable)
        sqlcon.Close()
        sqlcon.ConnectionString = strconnection()
        If sqlcon.State = ConnectionState.Closed Then
            sqlcon.ConnectionString = strconnection()
    ...
    Public Function JournalFormat(number As Double)
        Return If(number = 0, "", FormatNumber(number, , , TriState.True))
    End Function

    Public Function ReportFormat(number As Double)
        Return If(number = 0, "-", FormatNumber(number, , , TriState.True))
    End Function
...
#Region "Convert To Words Function"
    ...
    Public Function AmountInWords(ByVal nAmount As String, Optional ByVal wAmount As String = vbNullString, Optional ByVal nSet As Object = Nothing) As String
        ...
        'Trap null values
        If IsNothing(wAmount) = True Then wAmount = String.Empty Else wAmount = IIf(InStr(wAmount.Trim.ToLower, "pesos"), wAmount.Trim, wAmount.Trim & " Pesos Only")
...
#End Region
    ...
    Public Sub isCurrencyFormat(keypress As KeyPressEventArgs)
        Select Case Asc(keypress.KeyChar)
            Case 8
            Case 13
            Case 46
            Case 47 To 57
            Case Else
                keypress.KeyChar = Nothing
        End Select
    End Sub

    Public Function isCurrencyFormat(textbox As MaterialSkin.Controls.MaterialSingleLineTextField)
        If (System.Text.RegularExpressions.Regex.IsMatch(textbox.Text, "^\d*(\.\d*)?$")) Then
            Return True
        Else
            MsgBox(textbox.Text & " is not in currency format")
            textbox.Clear()
            Return False
        End If
    End Function

    Public Sub CurrencyFormat(materialtextbox As MaterialSkin.Controls.MaterialSingleLineTextField)
        If Not String.IsNullOrWhiteSpace(materialtextbox.Text) Then
                materialtextbox.Text = FormatNumber(materialtextbox.Text.Replace(",", ""), 0)
                materialtextbox.SelectionStart = materialtextbox.TextLength
        End If
    End Sub

    Public Sub CurrencyFormat(textbox As TextBox)
        If Not String.IsNullOrEmpty(textbox.Text) Then
            textbox.Text = FormatNumber(textbox.Text.Replace(",", ""), 0)
